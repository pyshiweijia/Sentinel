...
public class GatewayFlowRuleController {
    ...
    private DynamicRuleProvider<List<GatewayFlowRuleEntity>> gatewayRuleProvider;
    @Autowired
    @Qualifier("gatewayFlowRuleNacosPublisher")
    private DynamicRulePublisher<List<GatewayFlowRuleEntity>> gatewayRulePublisher;
    ...
    public Result<List<GatewayFlowRuleEntity>> queryFlowRules(String app, String ip, Integer port) {
        ...
        try {
            List<GatewayFlowRuleEntity> rules = gatewayRuleProvider.getRules(app);
    ...
    public Result<Long> deleteFlowRule(Long id) {
        ...
        if (!publishRulesToNacos(oldEntity.getApp())) {
    ...
    // 新增发布到Nacos的方法
    private boolean publishRulesToNacos(String app) {
        try {
            List<GatewayFlowRuleEntity> rules = repository.findAllByApp(app);
            gatewayRulePublisher.publish(app, rules);
            return true;
        } catch (Exception e) {
            logger.error("publish gateway flow rules to Nacos error, app={}", app, e);
            return false;
        }
    }
}
