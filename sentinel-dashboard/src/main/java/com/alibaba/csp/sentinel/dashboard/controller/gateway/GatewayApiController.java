...
public class GatewayApiController {
    ...
    @Autowired
    @Qualifier("gatewayApiDefinitionNacosProvider")
    private DynamicRuleProvider<List<ApiDefinitionEntity>> gatewayRuleProvider;
    @Autowired
    @Qualifier("gatewayApiDefinitionNacosPublisher")
    private DynamicRulePublisher<List<ApiDefinitionEntity>> gatewayRulePublisher;
    ...
    public Result<List<ApiDefinitionEntity>> queryApis(String app, String ip, Integer port) {
        ...
        try {
            List<ApiDefinitionEntity> apis = gatewayRuleProvider.getRules(app);
    ...
    public Result<Long> deleteApi(Long id) {
        ...
        if (!publishApisToNacos(oldEntity.getApp())) {
    ...
    // 新增发布到Nacos的方法
    private boolean publishApisToNacos(String app) {
        try {
            List<ApiDefinitionEntity> rules = repository.findAllByApp(app);
            gatewayRulePublisher.publish(app, rules);
            return true;
        } catch (Exception e) {
            logger.error("publish gateway apis to Nacos error, app={}", app, e);
            return false;
        }
    }
}
